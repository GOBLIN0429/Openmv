...
yellow_threshold = (0, 100, -32, 67, 30, 127)
...
while True:
    ...
    if WORK_STATE == 1:
        ...
        if mission_step == 1:
            ...
            #img.median(1)           # 去噪
            # 查找色块
            blobs = img.find_blobs([yellow_threshold],area_threshold= 1200 ,margin = 50,merge = True)
            max_blob = find_max_blob(blobs, 1500)
            #如果找到黄色色块
            if max_blob:
                green_led.on()
                ...
                if cx_filtered < YELLOW_WINDOW_LEFT:
                    #在画面左端，需左移
                    UART3_sendMyPack(0,0,1,0,0,0,0,0)
                    print("偏左")
                elif cx_filtered > YELLOW_WINDOW_RIGHT:
                    #在画面右端，需右移
                    UART3_sendMyPack(0,0,0,1,0,0,0,0)
                    print("偏右")
                elif cy_filtered < YELLOW_WINDOW_TOP:
                    #在画面上端，需上移
                    UART3_sendMyPack(0,0,0,0,1,0,0,0)
                    print("偏上")
                elif cy_filtered > YELLOW_WINDOW_BOTTOM:
                    #在画面下端，需下移
    ...
    if WORK_STATE == 2:
        ...
        #画出标识线
        if mission_step == 0:
            img.draw_rectangle(240, 160, 160, 160, (0,0,255),2)
            # 查找二维码
            for code in img.find_qrcodes():
                ...
                last_cx = cx_filtered
                last_cy = cy_filtered
                img.draw_cross(cx_filtered, cy_filtered, color=(0, 255, 0), size=10) # 红色中心点，大小为5像素

                if cx_filtered < QR_LEFT:
                    #在画面左端，需左移
                    UART3_sendMyPack(0,0,1,0,0,0,0,0)
                    print("偏左")
                elif cx_filtered > QR_RIGHT:
                    #在画面右端，需右移
                    UART3_sendMyPack(0,0,0,1,0,0,0,0)
                    print("偏右")
                elif cy_filtered < QR_TOP:
                    #在画面上端，需上移
                    UART3_sendMyPack(0,0,0,0,1,0,0,0)
                    print("偏上")
                elif cy_filtered > QR_BOTTOM:
                    #在画面下端，需下移
                    UART3_sendMyPack(0,0,0,0,0,1,0,0)
                    print("偏下")
                else:
                    ...
                    if counter_IS_IN == 3:
                        ...
                        time.sleep(0.5)
                        #拍照
                        file_name = "img_QR_code_%d"%counter_IS_IN
                        img.save(file_name)
                        counter_IS_IN += 1
                blue_led.off()
